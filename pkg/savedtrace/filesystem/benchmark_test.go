...
func BenchmarkRegressionMatrixTraceFilesystemGroupCommit(b *testing.B) {
	directory := b.TempDir()
	...
	for b.Loop() {
		...
		for index := range records {
			sequence++
			records[index] = benchmarkFilesystemRecord(fmt.Sprintf("trace-%d", sequence), started)
		}
...
func benchmarkFilesystemRecord(identifier string, started time.Time) savedtrace.Record {
